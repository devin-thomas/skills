...
If the chosen authoring path is unavailable, explain briefly and keep a Markdown/Mermaid model unless the user forbids that fallback. Retain the preferred format for later migration. Do not demand installation or authentication to continue discovery, fabricate an artifact URL, or claim an external diagram is synchronized when an update failed.
